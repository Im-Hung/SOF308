...
async function fetchReactions() {
  try {
    const response = await fetch(
      `http://localhost:3001/reactions?postId=${props.postId}`
    );
...
async function fetchUserReaction() {
  ...
  try {
    const response = await fetch(
      `http://localhost:3001/reactions?postId=${props.postId}&userId=${currentUser.value.id}`
    );
...
async function updateReaction(type) {
  ...
  if (reactions.length > 0) {
    ...
    const updateResponse = await fetch(
      `http://localhost:3001/reactions/${reactionId}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
